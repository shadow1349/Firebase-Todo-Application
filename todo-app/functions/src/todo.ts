...
export const OnListCreate = functions.firestore
    ...
    .onCreate((snapshot, context) => {
      ...
      // Run transaction to update the list countÍ for the user
    ...
    .onDelete(async (snapshot, context) => {
      ...
      await admin.firestore().runTransaction((transaction) =>
        transaction.get(userRef).then((user) => {
          ...
          // This shouldn't happen, but if we get a list count
          // below 0 we will reset it to 0
      ...
      return admin
          ...
          .recursiveDelete(
              admin.firestore()
                  .collection(`Lists/${context.params["ListId"]}/Items`)
          );
    });
...
export const OnItemCreate = functions.firestore
    ...
    .onCreate((snapshot, context) => {
      const listRef = admin.firestore()
          .doc(`Lists/${context.params["ListId"]}`);
